...
CLASS_WEIGHTS = tf.constant([5.0, 5.0, 1.0], dtype=tf.float32)
...
def get_data_paths(clean_dir, mask_dir):
    """Finds and returns lists of image and corresponding mask file paths."""
    image_files = []
    mask_files = []
    case_to_files = {}  
    ...
    for case_dir in sorted(clean_dir.iterdir()):
        if case_dir.is_dir():
            ...
            case_images = sorted(list(case_dir.glob('*.png')))
            ...
            if mask_case_dir.exists():
                ...
                for img_file in case_images:
                    ...
                    mask_name = f"{img_stem}_mask{img_suffix}"  
                    mask_file = mask_case_dir / mask_name
                    
                    if mask_file.exists():
                        case_pairs.append((img_file, mask_file))
                
                if case_pairs:
                    case_to_files[case_id] = case_pairs
                    for img, mask in case_pairs:
                        image_files.append(img)
                        mask_files.append(mask)

    print(f"Found {len(image_files)} valid image-mask pairs across {len(case_to_files)} cases")

    if not image_files:
        raise FileNotFoundError("Could not find any valid image-mask pairs. Please check the paths.")

    return image_files, mask_files, case_to_files

def split_data(image_paths=None, mask_paths=None):
    """
    Splits data into training and validation sets using predefined case assignments.
    Uses fixed folder assignments to ensure reproducibility and consistent data division.
    
    Args:
        image_paths: Not used, kept for compatibility
        mask_paths: Not used, kept for compatibility
        
    Returns:
        Tuple of (train_data, val_data) where each is (images, masks)
    """
    train_cases = ['4', '3', '1', '5', '6', '7', '13', '9', '10', '11', '12',
    ...
    _, _, case_to_files = get_data_paths(CLEAN_PATH, MASK_PATH)
        
    train_cases = [case for case in train_cases if case in case_to_files]
    val_cases = [case for case in val_cases if case in case_to_files]
    
    print(f"Fixed split: {len(train_cases)} train, {len(val_cases)} validation cases")
    
    train_images, train_masks = [], []
    val_images, val_masks = [], []
    
    for case in train_cases:
        if case in case_to_files:
            for img, mask in case_to_files[case]:
    ...
    train_pct = len(train_images) / total_images * 100 if total_images > 0 else 0
    val_pct = len(val_images) / total_images * 100 if total_images > 0 else 0
    
    print(f"Training:   {len(train_images)} images ({train_pct:.1f}%) from cases {', '.join(train_cases)}")
    print(f"Validation: {len(val_images)} images ({val_pct:.1f}%) from cases {', '.join(val_cases)}")
...
def asymmetric_focal_loss(y_true, y_pred, gamma_pos=2.0, gamma_neg=4.0):
    """
...
def main():
    ...
    (train_imgs, train_msks), (val_imgs, val_msks) = split_data(None, None)
    ...
    model.compile(optimizer=AdamW(learning_rate=lr, weight_decay=weight_decay), loss=asymmetric_focal_loss, metrics=metrics)
